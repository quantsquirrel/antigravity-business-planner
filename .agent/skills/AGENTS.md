...
## Purpose
Specialized capability packages that activate based on context. Contains 10 core skills (direct directories) and 3 extended skills (symlinks to `.agents/skills/`). Each skill has a `SKILL.md` defining its role, and some include `scripts/` with Python automation.
