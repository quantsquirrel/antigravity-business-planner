...
Step A(문제 탐색 3문) → Step B(솔루션/팀 2문) → 키워드 추출
  → AI 키워드 감지? → Yes → AI 보충 질문 3개 → AI 유형 분류 + 이중 스코어링
                     → No → 기존 플로우 유지
  ...
  → [AI 감지 시: AI 분석 보고서 생성] → [마인드맵 시각화]
```

### AI 사업 감지 분기 (v2.0 Phase 6)

키워드 추출 단계에서 AI 관련 키워드(AI, 자동화, 데이터, 인공지능, 머신러닝, 챗봇, LLM, GPT, 딥러닝, 자연어처리)가 감지되면:
1. `ai-business-analyst` 스킬이 연동되어 보충 질문 3개를 추가합니다
2. AI 비즈니스 유형을 분류하고, 유형별 수익 모델/리스크/방어력을 분석합니다
3. 기존 5점 평가에 AI 특화 리스크가 반영됩니다 (이중 스코어링)
4. Go 판정 시 `output/ideas/{id}/ai-analysis.md`가 추가 생성됩니다
5. AI 키워드 미감지 시 기존 프로세스가 그대로 유지됩니다
...
AI가 생성한 아이디어들의 다양성이 낮을 때(유사도 >0.85) 자동으로 SCAMPER 기법을 적용합니다.

**실행 규칙:**
| 규칙 | 설정값 |
|------|--------|
| 트리거 조건 | 생성된 아이디어 간 유사도 >0.85일 때만 |
| 적용 대상 | 최고 점수 아이디어 1개에만 |
| 아이디어당 최대 변형 | 2회 |
| 전체 후보 상한 | 15개 |
| 중복 제거 | 유사도 >0.85 자동 필터 |
| 대화형 모드 | 자동 트리거 허용 |
| 배치/무인 모드 | 기본 비활성, `--scamper` 플래그로만 활성화 |

> SCAMPER 7원칙: Substitute(대체), Combine(결합), Adapt(적용), Modify(변형), Put to other uses(용도변경), Eliminate(제거), Reverse(역발상)

### Human-in-the-Loop

아이디어 생성 후, 스코어링 **전**에 사용자 확인 단계를 거칩니다:
1. AI가 생성한 아이디어 목록을 사용자에게 표시
2. "아이디어 목록을 확인하세요. 제거하거나 수정할 항목이 있나요?"
3. 사용자가 부적절한 아이디어를 삭제/수정 가능
4. 확인 후 스코어링 단계 진행
...
## 사업 규모 설정 (v2.0 Phase 7)

idea-discovery 시작 시 사업 규모를 확인합니다. Step A 인터뷰 시작 전에 아래 질문을 합니다:

"어떤 규모의 사업을 계획하고 계시나요?"
1. 🧑‍💻 1인 개발/Micro-SaaS (혼자서 만들고 운영)
2. 👥 소규모 팀 (2-5명)
3. 🚀 스타트업 (투자 유치 목표)
4. 🏢 기존 사업/대규모

* 선택에 따라 idea.json의 `business_scale` 필드에 값을 저장합니다: `"micro"`, `"small"`, `"startup"`, `"enterprise"`
* 미선택("그냥 진행") 시 business_scale을 설정하지 않고 기존 프로세스 그대로 진행합니다
* `business_scale`이 `"micro"` 또는 `"small"`인 경우 `solo_builder: true`도 함께 설정합니다 (1인일 때)

### 규모별 평가 기준 분기 (Phase 7)

business_scale이 `"micro"` 또는 `"small"`인 경우, 5점 척도 평가에서 아래와 같이 해석을 조정합니다:

| 기존 기준 (startup/enterprise) | micro/small 기준 |
|-------------------------------|-----------------|
| 시장 크기 (TAM 기준) | 니치 시장 깊이 + 예상 전환율 |
| 경쟁 강도 (대기업 대비) | 기존 솔루션 불만도 + 전환 비용 |
| 창업자-문제 적합성 | 1인 실행 가능성 + 기술 역량 |
| 자원 (자금 중심) | 도구 접근성 + 시간 투자 가능량 |
| 타이밍 | 니치 커뮤니티 성장세 + 미충족 수요 |
